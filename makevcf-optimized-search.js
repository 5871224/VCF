...
// Adds an isolated, pre-warmed multi-worker VCF search and speed comparison.
// Every worker uses the untouched original eval/worker.js search engine.
(function initOptimizedVCFSearch() {
    ...
    function startAfterLayout() {
        ...
        class ParallelVCFEngine {
            constructor() {
                this.rules = Number(document.querySelector('input[name="rules"]:checked')?.value || 2);
                this.workerCount = Math.max(1, Math.min(4, navigator.hardwareConcurrency || 4));
                this.workers = [];
            ...
            depthPlan() {
                if (this.workerCount <= 1) return [200];
                if (this.workerCount === 2) return [31, 200];
                if (this.workerCount === 3) return [15, 47, 200];
                return [15, 31, 63, 200];
            }

            createWorker(index) {
                const worker = new Worker("eval/worker.js");
                worker.__vcfIndex = index;
                ...
                worker.onerror = event => {
                    console.error("Parallel VCF worker error", event);
                    const pending = this.pending.get(worker);
                    if (!pending) return;
                    this.pending.delete(worker);
            ...
            post(worker, cmd, param) {
                return new Promise(resolve => {
                    this.pending.set(worker, { resolve });
                    worker.postMessage({ cmd, param });
                });
            }
            ...
            async restart() {
                ...
                await Promise.all(this.workers.map(worker =>
                    this.post(worker, "setGameRules", { rules: this.rules })
                ));
            }
            ...
            async findVCF({ arr, color, maxNode = 5000000 }) {
                await this.ensureReady();
                const currentRun = ++this.runId;
                const depths = this.depthPlan();
                const completed = [];
                let settled = false;

                return new Promise(resolve => {
                    const finish = (result, winningDepth, reason) => {
                        if (settled || currentRun !== this.runId) return;
                        settled = true;
                        const winnerNodes = Number(result?.nodeCount || 0);
                        const completedNodeCount = completed.reduce((sum, item) => sum + Number(item.result?.nodeCount || 0), 0);
                        resolve({
                            ...(result || { winMoves: [], nodeCount: 0 }),
                            nodeCount: winnerNodes,
                            completedNodeCount,
                            optimizedMode: "parallel-depth-race",
                            parallelWorkers: depths.length,
                            winningDepth,
                            finishReason: reason,
                        });
                        this.ready = this.restart();
                    ...
                    let remaining = depths.length;
                    depths.forEach((maxDepth, index) => {
                        const worker = this.workers[index];
                        this.post(worker, "findVCF", {
                            arr: Array.from(arr),
                            color,
                            maxVCF: 1,
                            maxDepth,
                            ...
                            if (settled || currentRun !== this.runId) return;
                            completed.push({ maxDepth, result });
                            remaining--;

                            const route = result?.winMoves?.[0] || [];
                            if (route.length) {
                                finish(result, maxDepth, "found");
                                return;
                            }

                            if (remaining === 0) {
                                const fullDepth = completed.find(item => item.maxDepth === 200) || completed[completed.length - 1];
                                finish(fullDepth?.result || result, fullDepth?.maxDepth || maxDepth, "not-found");
                            }
                        });
                    });
                });
        ...
        const optimizedEngine = new ParallelVCFEngine();
        ...
        comparison.innerHTML = `
            ...
            <div class="vcf-speed-row"><strong>優化版</strong><span id="vcf-speed-optimized">引擎預熱中</span></div>
        ...
        style.textContent = `
            ...
            .vcf-speed-title { margin-bottom: 5px; font-weight: 700; font-size: 13px; }
        ...
        function renderComparison() {
            ...
            if (results.optimized) document.getElementById("vcf-speed-optimized").textContent = resultText(results.optimized);
        ...
        const statusObserver = new MutationObserver(() => {
            ...
            results.original = { ...parsed, fingerprint: originalPending.fingerprint };
        ...
        optimizedEngine.ensureReady().then(() => {
            setOptimizedButtonsDisabled(false);
            document.getElementById("vcf-speed-optimized").textContent = `尚未測試｜${optimizedEngine.workerCount} 個 Worker 已預熱`;
        }).catch(error => {
            console.error(error);
            document.getElementById("vcf-speed-optimized").textContent = "優化引擎初始化失敗";
        });
        ...
        async function runOptimized(color) {
            ...
            try {
                // Worker loading and evaluator initialization are excluded from search timing.
                await optimizedEngine.ensureReady();
                setStatus(`正在使用 ${optimizedEngine.workerCount} 個已預熱 Worker 搜索 ${colorName} VCF...`);
                const startedAt = performance.now();
                const info = await optimizedEngine.findVCF({ arr, color, maxNode: 5000000 });
                if (!info) {
                ...
                const mode = `並行 ${info.parallelWorkers} Worker，勝出深度 ${info.winningDepth}`;

                if (found) {
                    lastVCFMoves = Array.from(route);
                    window._showVCF(lastVCFMoves, color);
                    document.getElementById("btn-block-vcf").disabled = false;
                    setStatus(`優化 ${colorName} VCF 找到，共 ${route.length} 手（${seconds.toFixed(6)}s，勝出 ${fmtNodes(nodes)}，${formatExactRate(nodes, seconds)}；${mode}）`);
                } else {
                    setStatus(`優化 ${colorName} VCF 未找到（${seconds.toFixed(6)}s，完整深度 ${fmtNodes(nodes)}，${formatExactRate(nodes, seconds)}；${mode}）`);
                }
        ...
        document.querySelectorAll('input[name="rules"]').forEach(radio => {
            radio.addEventListener("change", async () => {
                if (optimizedBusy) return;
                setOptimizedButtonsDisabled(true);
