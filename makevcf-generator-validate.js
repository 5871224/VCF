...
function genAnalyzeVCFGroup(initialBoard, moves, attacker) {
  ...
  for (let i = 0; i < moves.length; i++) {
    ...
    if (idx < 0 || idx >= 225 || board[idx] !== GEN_EMPTY) {
      return { valid: false, steps: Infinity, levels };
    }
    board[idx] = color;
    if (color === attacker) {
      const level = getLevelPoint(idx, attacker, board) & 0x0f;
      levels.push(level);
      if (level === GEN_FOUR_NOFREE || level === GEN_FOUR_FREE) steps++;
      if (level >= GEN_FIVE) break;
    }
  }
  return { valid: true, steps, levels, completedBoard: board };
}

function genMatchesBaseContinuation(candidate, moves) {
...
async function genValidateCandidate(candidate) {
  ...
  for (const moves of groups) {
    ...
    if (!target && analysis.steps === 2 && genMatchesBaseContinuation(candidate, moves)) {
