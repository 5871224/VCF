...
- 一般候選驗證使用新版 `shortest + strict`，只取得目前最短的一批 VCF。
- 搜尋最大深度依目標步數限制為 `2 × 目標步數 + 1` ply，節點上限為 5,000,000。
- 未啟用補守子時，存在比目標短的 VCF 就淘汰候選。
- 啟用補守子時，先封鎖目前最短 VCF，再重新搜尋，直到最短層等於目標並符合預期完成盤面。
- 「只保留目標 VCF」使用新版 `multi + strict`，最多 64 組；搜尋中止或達上限時不得宣告唯一。
- 題目驗證固定使用嚴格剪枝，不得繼承主頁面的高速剪枝選項。
...
- 一般驗證確實使用 `shortest + strict`。
- 唯一題驗證確實使用 `multi + strict`，且達限制時不誤判唯一。
